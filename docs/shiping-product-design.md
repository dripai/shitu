...
| 全局快捷键与托盘 | 已实现 | F10 开始、F11 暂停/继续、F12 停止；托盘可恢复主窗口并控制录制 |
...
- 输出目录、来源、清晰度、帧率、声音、鼠标、倒计时和自动最小化设置持久化到 `%APPDATA%\ShiPing\config.json`。
- 托盘使用 Slint `SystemTrayIcon`；全局快捷键使用 `global-hotkey` 0.8.0。三个快捷键作为一组注册，任一键被占用时会注销已注册项并在主界面明确报错，不保留部分可用状态。
...
- [`global-hotkey` 0.8.0 `HotKey`](https://docs.rs/global-hotkey/0.8.0/global_hotkey/hotkey/struct.HotKey.html)
