...
use audio::SourceKind;
use config::Config;
use recorder::{Command, Event, RecorderHandle, RecordingOptions};
use target::{Bounds, RecordingTarget, WindowCandidates};
...
struct AppState {
    ...
    pending_options: Option<RecordingOptions>,
    countdown_token: u64,
...
fn bind_callbacks(main: &MainWindow, state: Rc<RefCell<AppState>>) {
    {
        let main = main.as_weak();
        let state = Rc::clone(&state);
...
fn bind_live_option_callbacks(main: &MainWindow, state: Rc<RefCell<AppState>>) {
    {
        let state = Rc::clone(&state);
...
fn open_target_selector(main: &MainWindow, state: &Rc<RefCell<AppState>>, mode: i32) -> Result<()> {
    if mode == 0 {
    ...
    let result = (|| -> Result<()> {
        ...
        if let Some(hwnd) = native_hwnd(main.window()) {
...
fn begin_countdown(main: &MainWindow, state: &Rc<RefCell<AppState>>) -> Result<()> {
    if state.borrow().recorder.is_some() || main.get_recording_state() != 0 {
...
fn recording_options(main: &MainWindow, state: &Rc<RefCell<AppState>>) -> Result<RecordingOptions> {
...
fn handle_recorder_events(main: &MainWindow, state: &Rc<RefCell<AppState>>) {
    let events = state
        .borrow()
        .recorder
        .as_ref()
...
fn begin_window_drag(window: &slint::Window) {
    let Some(raw) = native_hwnd(window) else {
        return;
    };
    let hwnd = HWND(raw as *mut _);
...
#[cfg(target_os = "windows")]
fn native_hwnd(window: &slint::Window) -> Option<isize> {
